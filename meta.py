...
class Meta():
    ...
    def __init__(self, args, xbase, ybase, xtarget, ytarget, ytargetadv, victim=False):
        print(
            f'Message from meta.py\'s constructor: we got bases of shape {xbase.shape}, base class of {ybase[0]}, target classes of {ytarget}, and adv classes of {ytargetadv}')
        self.victim = victim
        self.args = args
    ...
    def build_metalearner_graph(self):
        ...
        with tf.variable_scope('poisons'):
            ...
            if self.victim:
                ...
                self.colorperts = tf.Variable(
                    tf.zeros([self.args.npoison, *self.args.gridshape, 3], dtype=self.floattype),
                    name='colorperts')  # shape [npoison, ncolorres, ncolorres, ncolorres, 3]
                ...
                poisoninputs_, eye, xrefmin, xrefmax = recolor(poisoninputs_, colorperts_,
                                                               name='poisoninputs_masked_recolored')
                poisoninputs_ = tf.add(poisoninputs_, perts_, name='poisoninputs_masked_perturbed')
                self.poisoninputs = poisoninputs_

        # define target
        inputsT, labelsT, labelsC = tf.constant(self.xtarget, dtype=self.floattype), tf.constant(self.ytargetadv,
                                                                                                 dtype=self.inttype), tf.constant(
            self.ytarget, dtype=self.inttype)
        ...
        for i in range(self.args.nadapt + 1):
            with tf.variable_scope('adapter-' + str(i)):
                ...
                inputs = tf.placeholder(name='cleaninputs', shape=[self.args.batchsize, 32, 32, 3],
                ...
                if i == 0: self.buffrop = [tf.assign(self.buffers0[key], self.buffers[key]) for key in
                                           self.buffers]  # op to update the base iteration's batchnorm buffer
            ...
            with tf.variable_scope('targeter-' + str(i)):
                ...
                if i == 0:
                    self.accT0 = accT
                    self.accC0 = tf.reduce_mean(
                        tf.cast(tf.equal(tf.argmax(softmax, axis=1), self.ytarget), dtype=self.floattype))
                    classes = {}
        ...
        with tf.variable_scope('trainop'):
            with tf.control_dependencies([self.xentT0, self.accT0, self.cwT0]):
                ...
                if self.args.optimizer == 'sgd':
                    optim = tf.train.GradientDescentOptimizer(self.lrnrate)
                elif self.args.optimizer == 'mom':
                    optim = tf.train.MomentumOptimizer(self.lrnrate, momentum=.9)
                else:
                    optim = tf.train.MomentumOptimizer(self.lrnrate, momentum=.9)
        ...
        if not self.victim:
            ...
            with tf.variable_scope('metagrad'):
                ...
                for metagrad, metavar in metagradients:
                    metagrad_accum = tf.get_variable(name=f'metagrad_accum_{tf_basename(metavar)}',
                                                     shape=metavar.shape.as_list(),
                                                     dtype=self.floattype, initializer=tf.zeros_initializer,
                                                     trainable=False)
                    self.metagrads_accum.append(metagrad_accum)
                    self.accumop.append(
                        tf.assign_add(metagrad_accum, metagrad, name=f'metagrad_accumop_{tf_basename(metavar)}'))
                    self.avg_metagrads.append(tf.placeholder(shape=metavar.shape.as_list(), dtype=self.floattype,
                                                             name=f'avg_metagrads_{tf_basename(metavar)}'))
                self.avg_metagrads = tuple(self.avg_metagrads)

            with tf.variable_scope('craftop'):
                # apply metagradient
                self.avg_metagradients = [(metagrad, metavar) for metagrad, metavar in
                                          zip(self.avg_metagrads, metavars)]
                ...
                with tf.control_dependencies([craftop]):
                    ...
                    clipped = [
                        tf.clip_by_value(self.colorperts[:, :, :, :, i], -self.args.epscolor[i], self.args.epscolor[i])
                        for i in range(3)]
                    eye = tf.tile(tf.expand_dims(eye, 0), [self.args.npoison, 1, 1, 1, 1])
                    clipped = [
                        tf.clip_by_value(eye[:, :, :, :, i] + clipped[i], xrefmin[i], xrefmax[i]) - eye[:, :, :, :, i]
                        for i in range(3)]
                    ...
                    zeroop = [tf.assign(metagrad_accum, tf.zeros_like(metagrad_accum),
                                        name=f'zeroop_{tf_basename(metagrad_accum)}') for metagrad_accum in
                              self.metagrads_accum]
    ...
    def global_initialize(self, args, sess):
        ...
        pretrain_weights = None  # 预设为 None
        if args.pretrain and os.path.exists(args.pretrain):
            print(f"Loading local pretrain weights from {args.pretrain}")
            try:
                with open(args.pretrain, 'rb') as f:
                    pretrain_weights = pickle.load(f)
                self.init_weights(sess, pretrain_weights)
            except Exception as e:
                print(f"Failed to load local weights: {e}")
                pretrain_weights = None  # 加载失败则重置
        elif args.pretrain:
            print(f"Warning: pretrain path specified but not found: {args.pretrain}")

        return pretrain_weights  # 返回加载的权重（或 None）
